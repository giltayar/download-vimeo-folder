...
let itemsData = null;
const downloadsByVideoId = new Map(); // videoId -> download array
let authToken = null;
...
page.on("response", async (response) => {
  ...
  try {
    ...
    const downloadsMatch = url.match(
      /\/videos\/(\d+)\/versions\/\d+\/downloads/,
    );
    if (downloadsMatch) {
...
if (!itemsData?.data?.length) {
  console.error(
    "Failed to discover videos. Make sure the URL is a valid Vimeo review page.",
  );
  process.exit(1);
}
...
for (const v of videos) {
  if (!downloadsByVideoId.has(v.videoId) && authToken && reviewId) {
    ...
    const item = itemsData.data.find((i) =>
      i.video?.uri?.endsWith(`/${v.videoId}`),
    );
    const versionUri =
      item?.video?.metadata?.connections?.versions?.current_uri;
...
const downloadTasks = videos.map((v, i) =>
  limit(async () => {
    ...
    if (!best?.link) {
      console.log(
        `[${i + 1}/${videos.length}] "${name}" — no download link, skipping`,
      );
      return;
    }
    ...
    if (existsSync(filepath)) {
      console.log(
        `[${i + 1}/${videos.length}] "${name}" — already exists, skipping`,
      );
      downloaded++;
      return;
    }
    ...
    const sizeMB = existsSync(filepath)
      ? (
          (await import("node:fs")).statSync(filepath).size / 1048576
        ).toFixed(1)
      : "?";
    console.log(
      `[${i + 1}/${videos.length}] "${name}" ✓ done (${sizeMB} MB)`,
    );
    downloaded++;
  }),
);
